...
def offer_editor():
    ...
    with cols[0]:
        st.session_state.adjusted_params["offer_name"] = st.text_input(
            ...
            help="Give your offer a catchy name!"
        )
        ...
        st.session_state.adjusted_params["value"] = st.number_input(
            ...
            key="value_input",
            help="The value of the discount/cashback"
        )
    ...
    with cols[1]:
        ...
        if st.session_state.adjusted_params.get("max_redemptions"):
            st.session_state.adjusted_params["max_redemptions"] = st.number_input(
                ...
                help="Maximum number of times this offer can be used"
            )
...
def display_offer(params):
    ...
    if st.session_state.lms_credentials['email'] and st.session_state.lms_credentials['password']:
        if st.button("🚀 Publish to LMS", help="Publish this offer to the LMS platform"):
            with st.spinner("🛰️ Publishing offer to LMS..."):
        ...
        st.warning("⚠️ Add LMS credentials to enable publishing")

# Main workflow
if st.button("✨ Generate Offer", help="Let AI create an offer based on your description") and user_prompt:
    with st.spinner("🧠 AI is crafting your perfect offer..."):
...
if st.session_state.offer_created and st.session_state.adjusted_params:
    st.success("🎛️ Adjust the offer below and see changes in real-time:")
